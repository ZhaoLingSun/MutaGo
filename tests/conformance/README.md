...
本目录用于 C++ 生产权威实现与独立 Python 参考实现之间的确定性一致性和差分测试。M0 已在 `tests/contracts/` 提供可执行合同示例与测试；本目录保留明确标为 **UNFROZEN v0、仅测试排练** 的 NORMAL/PASS C++ JSONL probe 与 Python 差分驱动，并提供与旧模式严格分离的 **UNFROZEN Double Increment 1**、**UNFROZEN Immortal Increment 2**、**UNFROZEN Eightway Increment 3**、**UNFROZEN Full-rule Diff v4** 和 **UNFROZEN Administrative-termination Diff v5** 测试载体。六种协议都不是生产协议、百万动作夹具、门槛报告或 CI 配置。
...
- 底层 `OracleConfig` / `PlayerQuotas` 接受满足状态一致性与配额守恒的非负 JSON-safe 整数；外部测试载体刻意更窄：legacy v0 只暴露 `quotaMode=ZERO/ONE`，Double Increment 1、Immortal Increment 2、Eightway Increment 3、Full-rule Diff v4 与 Administrative-termination Diff v5 的 `initialQuotas` 每个分量分别只允许 `0..4`；这六个 carrier 范围都不能倒推为 oracle 的配额域；
...
- 稳定 `COLLAPSE_PLAY` / `ORDINARY_PLAY` 边界上的 `RESIGNATION` / 游戏语义 `TIMEOUT`，包括 pending Double、任一具名败方、终局优先拒绝、原状态逐字段保留、source-aware typed terminal event 与一次不改变占据的 PSK 追加；
...
PYTHONPATH=python python3 -m unittest -v \
  ...
  tests/conformance/test_python_administrative_termination.py
```

这些测试证明的是共享 Python oracle 的本地完整特殊能力、计分与管理终止行为；它们不提供生产管理命令 transport、持久化、搜索或产品路径证据，也不表示 `GATE-RULE-1M` 或 `GATE-PROD` 已完成或通过。
...
## Administrative-termination Diff v5 一致性载体：UNFROZEN，仅测试

第六个独立模式使用协议字面值 `administrative-termination-diff-v5-unfrozen`。`administrative_termination_differential.py` 和它的全部字段、状态投影、生成器及 transcript 都明确为 **UNFROZEN** 测试载体；它不是 M0 Schema、生产命令协议、权威日志格式、持久化格式、时钟或超时证据协议。C++ probe 只调用真实 `CollapseGoReducer::apply` 或 `CollapseGoReducer::terminate`，不直接改写规则状态；Python 预期只调用独立 `apply_action` 或 `apply_administrative_termination` 预言机 API，且不调用 C++、不复用 C++ 输出，也不通过逐动作 reducer sweep 生成预期。

关闭式请求继续恰含 `protocolVersion`、`episodeId`、`boardSize`、`initialQuotas`、`steps`。每一步恰含 `candidateActor` 与 `candidate`：`ACTION` candidate 恰含 `kind` 和嵌套、未改形的关闭式 Action V1 `action`；`RESIGNATION`、`TIMEOUT` candidate 只含 `kind`。`ACTION` 的 `candidateActor` 是动作行动者；两个管理 candidate 的 `candidateActor` 是具名败方，因此不受当前行动者限制，黑白任一方都可在同一稳定边界成为败方。载体没有 race、timestamp、priority 或并发字段；先后关系只由单一有序 candidate 序列表达。

v5 使用专门的 admin-capable 稳定状态投影，不改动 v0-v4 的历史状态或 transition 投影。该投影显式包含 `settlementCompleted`、关闭式终局 reason/winner/loser 与 nullable score、pending Double 审计链接、完整 source-aware stones、mixed groups、全局 ledger、四组配额、动作/PASS/settlement/terminal/revision/log counters，以及有序 occupancy-only PSK。完整 `legalActionRanges` 只允许出现在 episode 初态和每个 candidate 后的最终稳定状态；atomic snapshot/event、settlement、removal batch 和 terminal event 均不得携带。Python 在每个这些稳定状态调用 `enumerate_action_legality` 与其公开 bool-mask 视图 `derive_legal_mask`；后者是前者的直接适配，不冒充第二套独立 Python 规则实现。载体要求两个公开视图的 1,445 项完全一致，再把 C++ 与 Python 范围分别展开为全部 1,445 bits 比较；跨语言比较才是独立规则实现证据，mask 差异报告首个不同的 `actionId`。

transition 是关闭式 union。`ACTION` 分支保留 v4/v3 的既有 atomic action、atomic snapshot、settlement 闭包和 SCORE terminal event 语义。接受的管理分支为 `IMMEDIATE_TERMINAL`：没有 action、atomic event、atomic snapshot、settlement 或 score，只有一个 typed terminal event 和一次未改变稳定占位的 PSK append。被拒绝的 ACTION 或管理 candidate 必须逐字段保持整个前态，事件为空且 PSK append 数为零。管理终止先到则之后动作拒绝；阈值动作、提前第二个 `PASS` 或到达 `T` 的 Double continuation 先到时，其动作与完整 settlement 已在同一次 API 调用内原子完成，管理 candidate 只在新的 `ORDINARY_PLAY` 稳定出口处理。第二个 ordinary `PASS` 先到则 SCORE 获胜，之后认输/游戏超时均以 `TERMINAL_STATE` 拒绝。

curated 语料覆盖两种原因、两种败方、非当前行动者败方、初态、普通 collapse、collapse 一次 PASS、pending Double、ordinary 初态和 ordinary 一次 PASS 边界；还覆盖管理/动作在 NORMAL、特殊动作和 PASS 前后的两种顺序、`A=T-1` 时管理终止先于最终阈值动作、`A=T-1` 的 pending Double 当前行动者 `TIMEOUT` 先于将到达 `T` 的 continuation、阈值动作、提前第二 PASS、分别以 NORMAL/PASS 到达 `T` 的 Double continuation、SCORE 后管理拒绝，以及 terminal 后重复管理/动作。genesis candidate reexecution 通过分别从新局执行 prefix、full 和 extended 序列来比较不可变前缀；它不是 checkpoint restoration，也不冒充 serialized checkpoint recovery、生产 replay 或持久化恢复。非对称含 I/D/E 的管理 episode 在 9×9、13×13、19×19 上覆盖全部八种 D4 与 inverse，并以可达的三种特殊动作、settlement 和管理终局锚点防止空洞等变测试。关闭式负例覆盖错误 candidate union、冗余 Action V1 字段、hostile raw frame、受限 canonical profile 和 v0-v5 混合 dispatch。进程 timeout、退出失败、stderr、输出溢出或传输失败只产生 harness `ProbeError`，绝不插入语义 `TIMEOUT` candidate 或生成胜负。

默认 seed 是 `mutago-administrative-termination-increment-5`；所有 seed 在构造 manifest 或随机流前都必须是 1–256 字节的可打印 ASCII，CLI 对非法 seed 的失败诊断使用有界 ASCII 转义表示，不能在错误处理阶段再次 canonicalize 失败。随机流使用与 v0-v4 不同的固定 domain `MutaGo administrative termination differential v5 UNFROZEN\0`。当前有界默认包含 475 个 curated candidate 和 64 个 deterministic random candidate，共 539 个 candidate、53 个 episode、592 次稳定状态合法性比较，即 855,440 个 legal-bit 比较；结果为 418 个 ACTION accepted、62 个 ACTION rejected、48 个管理 accepted 和 11 个管理 rejected。这些数目远低于一百万，且不能与历史载体累加为 gate 证据。validator 会从 genesis 重新执行每条 Python expected episode，并以 exact recursive type/shape/value 比较拒绝 expected/actual 同时伪造的 ACTION 接受性、合法范围、捕获顺序/重复、状态、事件和整数/布尔类型；现有 v3 深层状态、atomic lineage 与 settlement 闭包检查继续叠加执行。进程入口在同一 absolute deadline 和可信异常包装内把 caller 提供的 request/expected sequence 各物化一次，随后序列化、响应绑定、诊断和 digest 只使用该不可替换快照；stdout 先计数换行，再以有界逐行扫描处理，不构造 `split` 产生的第二份完整行列表。诊断由 harness 的可信上下文生成：response/completed counts、manifest、canonical request、candidate prefix、精确 pre-candidate Python 状态和当前 candidate 均位于不可信失败文本之前；header 与初态失败归因于 prefix 0，final transcript digest deadline 也保留同一完整可信上下文。completed count 只统计完整通过 canonical、形状和语义验证的响应前缀，若 stdout 本身无法按 UTF-8 解码则保守报告零个已验证响应而不把原始换行数冒充验证结果。transcript 继续对 manifest、每条 canonical request 和 response 使用 8 字节大端长度前缀后做 SHA-256。修复后的真实 probe 完整默认执行已连续两次得到逐字段相同摘要，v5 有界 transcript 固定为 `b77d2053bfd3474e6e5c772e2369473202e6bb3bcb90fde21940eddc3b6206da`；v0-v4 已审阅 digest 原样保持。
...
python3 -m unittest -v \
  tests/conformance/test_administrative_termination_differential.py
```

可执行 probe 可在仓库外构建；opt-in 集成和 CLI 使用独立环境变量/入口：

```bash
BUILD_DIR="${TMPDIR:-/tmp}/mutago-collapse-administrative-v5"
cmake -S cpp -B "$BUILD_DIR" -DUSE_BACKEND= -DCMAKE_BUILD_TYPE=RelWithDebInfo
cmake --build "$BUILD_DIR" --target mutago-collapse-slice-probe -j4

MUTAGO_COLLAPSE_ADMINISTRATIVE_TERMINATION_PROBE="$BUILD_DIR/mutago-collapse-slice-probe" \
PYTHONDONTWRITEBYTECODE=1 PYTHONWARNINGS=error PYTHONPATH=python \
python3 -m unittest -v \
  tests/conformance/test_administrative_termination_differential.py

PYTHONDONTWRITEBYTECODE=1 PYTHONWARNINGS=error PYTHONPATH=python \
python3 tests/conformance/administrative_termination_differential.py \
  --probe "$BUILD_DIR/mutago-collapse-slice-probe" \
  --seed mutago-administrative-termination-increment-5 --candidate-count 64
```

成功摘要的 scope 只能是 `ADMINISTRATIVE_TERMINATION_DIFF_V5_UNFROZEN_TEST_ONLY`，并明确带 `unfrozenTestOnly=true`、`gateRule1MClaimed=false`、`gateProdClaimed=false` 和 `operationalFailureSynthesizesSemanticTimeout=false`。该载体不提供生产命令 envelope、并发调度、时间控制证据、JSON 权威日志 replay、崩溃恢复、undo/redo、sanitizer、搜索、模型、数据 producer、Gateway/Web、来源或发行审计证据，不构成 `GATE-RULE-1M`、`GATE-PROD`、release 或审计完成声明；项目仍为 **AUDIT-BLOCKED**。
...
A sixth separate test-only protocol, `administrative-termination-diff-v5-unfrozen`, exercises the frozen resignation and semantic-timeout ordering rules without changing any v0-v4 request, response, projection, generator, or digest. Its closed request keeps `protocolVersion`, `episodeId`, `boardSize`, `initialQuotas`, and `steps`; each step contains only `candidateActor` and a closed `candidate` union. `ACTION` contains `kind` plus an unchanged nested Action V1, while `RESIGNATION` and `TIMEOUT` contain only `kind`; for administration, `candidateActor` names the loser and may be either color regardless of the current actor. Ordering is represented solely by candidate sequence order. The C++ probe calls the real reducer `apply` or `terminate` API and never mutates the state directly. Python expectations call only the independent `apply_action` or `apply_administrative_termination` API and never C++, C++ output, or an action sweep. A dedicated admin-capable stable projection includes explicit `settlementCompleted`, typed terminal reason/winner/loser with nullable score, pending-Double audit state, source-aware stones/groups, complete ledger/quota/counter data, and ordered occupancy-only PSK. Full `legalActionRanges` occur only at the initial and every post-candidate stable state; both Python legality APIs must agree and all 1,445 bits are compared at every such state. ACTION retains the existing atomic transition shape and is independently checked with the v3 state, topology, lineage, settlement, score, and PSK invariants. Accepted administration is exactly `IMMEDIATE_TERMINAL`, with no action, atomic event/snapshot, settlement, or score, one terminal event fully bound to the committed state, and one unchanged-occupancy PSK append; every rejected candidate preserves exact state and emits no event or append. Curated sequences cover both reasons and losers, non-current losers, initial/collapse/one-pass/pending-Double/ordinary/one-pass-ordinary boundaries, NORMAL/special/PASS ordering, termination-first at `A=T-1` both before the final threshold action and from a pending Double whose continuation would reach `T`, threshold and early-pass settlement, NORMAL and PASS Double continuations reaching T, SCORE winning before later admin rejection, terminal repeats, genesis prefix/full/extended reexecution, and all eight D4 transformations plus inverses on 9/13/19 with explicit reachable I/D/E, settlement, and terminal anchors. Genesis reexecution is not checkpoint restoration, serialized recovery, production replay, or persistence evidence; those claims remain outside this carrier. Strict restricted-profile parsing, semantic validation, resource limits, and process supervision are reused from checkout-pinned hardened modules without global mutation. Seeds are validated before manifest construction as 1–256 bytes of printable ASCII, and CLI failures render invalid inputs through a bounded ASCII-escaped diagnostic rather than recursively failing canonicalization. Caller request/expected sequences are snapshotted once under the same absolute deadline and trusted exception wrapper; stdout line counts and frames are scanned without materializing a second `split` list. Trusted failures put response/completed counts, manifest, canonical request, candidate prefix, exact pre-candidate Python state, and candidate before untrusted failure text; header/initial-state failures use prefix zero, final-digest deadline failures retain the same context, and completed counts include only fully validated response prefixes. Operational probe timeout/failure raises only a harness error and never synthesizes semantic `TIMEOUT`. The bounded default has 475 curated plus 64 random candidates across 53 episodes, with 418 ACTION accepts, 62 ACTION rejects, 48 administrative accepts, 11 administrative rejects, 592 stable-state mask comparisons, and 855,440 legal-bit comparisons. Two consecutive complete executions of the repaired real probe produced the identical transcript SHA-256 `b77d2053bfd3474e6e5c772e2369473202e6bb3bcb90fde21940eddc3b6206da`; all v0-v4 pins remain unchanged. The carrier is explicitly `ADMINISTRATIVE_TERMINATION_DIFF_V5_UNFROZEN_TEST_ONLY`, claims neither gate, does not define a production protocol or timeout evidence policy, and leaves the project **AUDIT-BLOCKED**.
